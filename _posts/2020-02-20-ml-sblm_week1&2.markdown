...
#### Summary  
```  
- how to use Monte Carlo methods to create GPI algorithms  
- showed an example algorithm, Monte Carlo with Exploring Starts
```  
...
#### Summary  
```  
- We showed how to apply Monte Carlo with Exploring Starts to solve an example MDP.  
```  
...
- 이 강의에서 배울 것  
  ```  
  - Understand the Exploring Starts can be problematic in real problems.  
  - Describe an alternative method to maintain exploration in Monte Carlo control  
  ```  
...
#### Summary  
```  
- how sampling an initial state-action pair for exploring starts is not always feasible.  
...
- 이 강의에서 할 것  
  ```  
  - Understand the off-policy learning can help deal with the exploration problem
  - Produce examples of target policies  
  - Produce exmaples of behavior policies  
  ```  
...
- The policy that we are choosing from is the behavior policy 
```  
...
- 이 강의에서 할 것  
  ```   
  - Use importance sampling to estimate the expected value of a distribution using samples from a different dist'n  
  ```  
...
#### Summary  
```  
- Importance sampling uses samples from one probability dist'n to estimate the expectation of a 
