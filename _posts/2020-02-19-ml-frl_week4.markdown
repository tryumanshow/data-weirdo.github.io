...
- Dynamic Programming은 다양한 Bellman equation (앞에서 봐왔던) & envronment `p`에 대한 지식을 함께 사용   
  (고전적인 DP는 ENVORNMENT와의 상호작용은 생각하지 않음. 대신 DP방법을, 주어진 MDP Model에 대해  
  value function과 optimal policies를 계산해내는 데에 사용)   
  `Dynamic programming uses the various Bellman equations we've seen, along with knowledge of p, to work`  
  `out value functions and optimal policies.`  
