...
- 강화학습 및 Multi-Armed Bandit 문제에 대한 간단한 소개와, 이를 해결하기 위한 알고리즘에 대한 소개입니다.  

---  

# 1. Fundamentals of Reinforcement Learning - Week1  
...
- 하지만 강화학습의 기초는 거의 같음  
  ...
  이 아이디어를 합쳐놓은 것  
  ```  
  - DQN이라는 하나의 learning system에서 강화학습 시스템의 가장 기본적인 초석들을 볼 수 있음  
...
- 이 강의에서 할 것  
  ...
  - Describe fundamental concepts: rewards, time steps, and values  
  ```  
  ```
...
- Agent(`Decision Maker`)  
  ...
  * Doctor가 최상의 Reward를 낳는 action을 선택할 것  
  ```  
  ```  
...
- `Value of an Action`  
  ...
  - `Sample-Average Method`  
    ...
    - ![](https://latex.codecogs.com/gif.latex?Q_*%28a%29%20%5Cdoteq%20%5Cfrac%7B%5Csum_%7Bi%3D1%7D%5E%7Bt-1%7DR_i%7D%7Bt-1%7D)  
      ...
      = (교재) ![](https://latex.codecogs.com/gif.latex?%5Cfrac%7B%5Csum_%7Bi%3D1%7D%5E%7Bt-1%7DR_i%5Ccdot%20%5Cmathbb%7BI%7D_%7BA_i%3Da%7D%7D%7B%5Csum_%7Bi%3D1%7D%5E%7Bt-1%7D%5Cmathbb%7BI%7D_%7BA_i%3Da%7D%7D)  
...
- 이 강의에서 할 것:  
  ...
  - Identify how the incremental update rule is an instance of more general update rule
  - Describe how the general update rule can be used to solve a non-stationary bandit problems  
  ```
...
- `Non-stationary bandit problem`  
  ...
  - `Decaying past rewards`  
    ...
    ![](https://latex.codecogs.com/gif.latex?%3D%20%281-%5Calpha%29%5EnQ_1%20&plus;%20%5Csum_%7Bi%3D1%7D%5En%7B%5Calpha%281-%5Calpha%29%5E%7Bn-i%7DR_i%7D)     ![](https://latex.codecogs.com/gif.latex?Q_1) ← initial action-value  
    ```  
...
- 이 강의에서 할 것   
  ...
  - Define epsilon greedy, a simple method to balance exploraation and exploitation  
  ```  
...
- 위 문제를 해결하기 위한 방법 중 하나: 
  ...
  - 대부분의 경우 Exploit, 적은 확률로 Exploring  
    (Behave greedily most of the time, but every once in a while, say with small probability ε,  
    instead select randomly from among all the actions with equal probability, independently of  
...
- 10-armed Testbed (on the textbook)  
  - 10 actions  
...
- 이 강의에서 할 것  
  ```  
  - Understand how optimistic initial values encourage early exploration  
  - Descrbie limitations of optimistic initial values  
  ```  
  ...
  - `한계`  
    ...
    2. non-stationary problem에는 적합하지 않음  
      - action value가 시간이 좀 지난 후에는 바뀔 수도 있는데, 
        optimistic agen는 이미 특정 액션에 지나치게 고착화되어버려서, 
...
- 이 강의에서 할 것  
  ```  
  - Understand how UCB action-selection uses uncertainty in estimates to drive exploration  
  ```  
...
- 예시 중에 이런 예시가 있음  
  ```  
  Case1. Epsilon of 0.1 perform better over 1000 steps than epsilon of 0.01  
  이유: 0.01 agent did not explore enough. Thus it ended up selecting a suboptimal arm for longer.  
  Case2. Epsilon of 0.0 (a greedy agent) perform better than epsilon of 0.4  
  이유: 0.4 agent explores too often that it takes many sub-optimal actions causing it to do worse over the long term  
  ```  
  
---  
---  
  
- Multi-Armed Bandit 문제에 대한 내 개인적인 정리  
  - K-Armed Bandit 문제란, agent가 K개의 선택지에 대해 각 선택에 대해 그에 상응하는 reward를 받고, 
    기대 reward를 maximize하는 방향으로 선택을 하려는 것  
  - 여기서 기대 reward: action value ![](https://latex.codecogs.com/gif.latex?q_*%28a%29)  
  - 하지만 확실한 action value를 이 기대 reward를 추정해야만 하고 그 estimator가 ![](https://latex.codecogs.com/gif.latex?Q_*%28a%29). 
    - `sample-average method`  
    - 재귀적으로 정의 가능    
      ```  
      - stationary   
      - non-stationary  
      ```  
  - exploitation-exploration 사이의 balancing 문제  
    - 방법론 소개  
    ```  
    (0. Greedy Method)  
