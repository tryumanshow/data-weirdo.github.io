...
- State Value Function 및 Action Value Function, 또 각각에 상응하는 Bellman Equation, Optimal Value Function에 대해 다루고 있는 글입니다.  
...
- Action-value functions represent the expected return from a given state after taking a specific action, 
...
- Optimal Value Functions  
  ...
  ```  
  Because it is the optimal value function, however, v*'s consistency condition can be written in a special 
  ...
  for an action that achieves the highest value, 
...
- 예시    
  ...
  - Left is the optimal action in this state, and must be selected by any optimal policy.  
